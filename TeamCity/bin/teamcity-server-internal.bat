...
SET TEAMCITY_SERVER_MEM_OPTS_ACTUAL=-Xmx1024m
...
SET custom_server_opts=%TEAMCITY_SERVER_OPTS% %TEAMCITY_SERVER_MEM_OPTS_ACTUAL%
SET CATALINA_OPTS=%custom_server_opts% -server
if exist "teamcity-server-java-agent.jar" (
   SET CATALINA_OPTS=%CATALINA_OPTS% -Xbootclasspath/a:teamcity-server-java-agent.jar -javaagent:teamcity-server-java-agent.jar
)
IF ""%2"" == ""service"" SET CATALINA_OPTS=%CATALINA_OPTS% -Xrs
SET CATALINA_OPTS=%CATALINA_OPTS% -Dteamcity.configuration.path=../conf/teamcity-startup.properties -Dlog4j2.configurationFile=file:../conf/teamcity-server-log4j.xml "-Dteamcity_logs=%TEAMCITY_LOGS_PATH%"
...
set _str="%custom_server_opts: =_%"
set _str=%_str:^"=_%
set _replaced=%_str:ReservedCodeCacheSize=%
IF %_replaced%==%_str% (
   set JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% -XX:ReservedCodeCacheSize=640M
)

:: Enable security manager explicitly for Java > 17 (21 and above)
CALL "%cd%\findJava.bat" fun version_ge %FJ_JAVA_VERSION% 17 <nul
IF ERRORLEVEL 0 set JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% -Djava.security.manager=allow
...
if ""%1"" == ""stop"" (
  ...
  call catalina.bat %1 %2 %3 %4 %5 %6 %7 %8 %9
)
