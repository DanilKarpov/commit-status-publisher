...
<c:choose>
  <c:when test="${errorCode == 'newTokenRequired'}">
    ${styles}
    <div>
      To show available repositories TeamCity requires access to your <a href="https://bitbucket.org/" target="_blank" rel="noreferrer">Bitbucket Cloud</a> account.
      <br/>
      <br/>
      <iframe hidden id="iframe_${oauthProvider.id}">
        <script>
          window.BitBucketRepositoriesContentUpdater = parent.BitBucketRepositoriesContentUpdater;
        </script>
      </iframe>
      <c:set var="onclick">
        ...
        var win = BS.Util.popupWindow(
          window['base_uri'] + '/oauth/bitbucket/repositories.html?projectId=${project.externalId}&connectionId=${oauthProvider.id}&vcsType=${vcsType}&updateToken=true&showMode=<%=WebUtil.encode(showMode)%>&userId=${currentUser.id}',
  ...
  <c:when test="${errorCode == 'tokenObtained'}">
    <bs:externalPage>
      ...
      <jsp:attribute name="head_include">
        ${styles}
        <script type="text/javascript">
          if (window.opener && window.opener.parent.BitBucketRepositoriesContentUpdater) {
            window.opener.parent.BitBucketRepositoriesContentUpdater();
          }
          window.close();
        </script>
      </jsp:attribute>
