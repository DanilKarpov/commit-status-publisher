  ...
  <c:when test="${errorCode == 'newTokenRequired'}">
    ...
    <div>
      ...
      <iframe hidden id="iframe_${oauthProvider.id}">
        <script>
          window.TfsRepositoriesContentUpdater = parent.TfsRepositoriesContentUpdater;
        </script>
      </iframe>
      <c:set var="onclick">
        var iframe = document.getElementById('iframe_${oauthProvider.id}');
        var win = BS.Util.popupWindow(
          window['base_uri'] + '/oauth/tfs/repositories.html?projectId=${project.externalId}&connectionId=${oauthProvider.id}&updateToken=true&showMode=<%=WebUtil.encode(showMode)%>&userId=${currentUser.id}',
  ...
  <c:when test="${errorCode == 'tokenObtained'}">
    <bs:externalPage>
      ...
      <jsp:attribute name="head_include">
        ...
        <script type="text/javascript">
          if (window.opener) {
            if (window.opener.parent.TfsRepositoriesContentUpdater) {
              window.opener.parent.TfsRepositoriesContentUpdater();
            }
            window.close();
          }
        </script>
